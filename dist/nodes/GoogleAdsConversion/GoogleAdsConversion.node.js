...
class GoogleAdsConversion {
    ...
    buildDetailedAuthErrorMessage(error, executeFunctions) {
        ...
        try {
            ...
            suggestions.push('1. **Re-authenticate**: Your OAuth2 token may have expired. Reconnect your Google Ads credentials in n8n.');
            suggestions.push('2. **Check Developer Token**: Ensure your Google Ads Developer Token is valid and approved for production use.');
            suggestions.push('3. **Verify Account Access**: Make sure the authenticated Google account has access to the target Google Ads account.');
            if (accountType === 'manager') {
                ...
                suggestions.push('4. **Account Type**: If you\'re using a manager account, change the account type to "Manager Account (MCC)".');
            }
            suggestions.push('5. **API Access**: Verify that your Google Cloud project has the Google Ads API enabled.');
            suggestions.push('6. **Billing**: Ensure your Google Ads account has active billing if required.');
            suggestions.push('\n**Debugging Steps:**');
            suggestions.push('- Enable debug mode to see detailed request information');
            suggestions.push('- Check the Google Ads account ID in your credentials matches the target account');
    ...
    validateInputParameters(executeFunctions, itemIndex) {
        ...
        switch (identificationMethod) {
            ...
            case 'enhanced':
                ...
                if (phoneNumber && !email && !firstName && !lastName) {
                    executeFunctions.logger.warn('Using only phone number for Enhanced Conversions. For better match rates, consider adding email or name fields. Enhanced Conversions work best when combined with GCLID or when multiple user identifiers are provided.');
                }
                break;
            default:
        ...
        try {
            ...
            const now = new Date();
            if (date > now) {
                throw new GoogleAdsValidationError(executeFunctions.getNode(), `Conversion date time cannot be in the future. Provided: ${conversionDateTime}, Current time: ${now.toISOString()}. Google Ads only accepts past conversion events.`, 'conversionDateTime');
            }
            // Check if date is too old (Google Ads has limits, typically 90 days)
            const ninetyDaysAgo = new Date(now.getTime() - 90 * 24 * 60 * 60 * 1000);
            if (date < ninetyDaysAgo) {
                executeFunctions.logger.warn(`Conversion date is older than 90 days (${conversionDateTime}). This may be rejected by Google Ads.`);
            }
        }
        catch (dateError) {
    ...
    async getAuthenticatedHeaders(executeFunctions) {
        try {
            ...
            if (!sanitizedLoginCustomerId) {
                executeFunctions.logger.warn('Invalid login-customer-id after sanitization. This will likely cause authentication issues.', { originalValue: loginCustomerId });
            }
    ...
    async validateCredentials(executeFunctions, headers) {
        ...
        if (!headers['developer-token']) {
            throw new GoogleAdsAuthenticationError(executeFunctions.getNode(), 'Developer token is missing. Please ensure your Google Ads credentials include a valid developer token.');
        }
        if (!headers['login-customer-id']) {
            throw new GoogleAdsAuthenticationError(executeFunctions.getNode(), 'Login customer ID is missing. Please ensure your Google Ads credentials include a valid customer ID.');
        }
        if (debugMode) {
        ...
        await this.executeWithRetry(executeFunctions, async () => {
            ...
            if (conversionAction && debugMode) {
                ...
                if (conversionActionId) {
                    const isValidAction = await this.testConversionAction(executeFunctions, customerId, conversionActionId);
                    if (!isValidAction) {
                        executeFunctions.logger.warn(`Conversion Action ${conversionActionId} may not be accessible or active. This could cause upload failures.`);
                    }
                }
            }
            return response;
        ...
        try {
            ...
            if (response.results && response.results.length > 0) {
                ...
                if (conversionAction.status !== 'ENABLED') {
                    console.warn(`Conversion Action ${conversionActionId} status is: ${conversionAction.status}. It should be ENABLED.`);
                    return false;
                }
                return true;
            }
            else {
                console.error(`Conversion Action ${conversionActionId} not found in customer ${customerId}`);
                return false;
            }
        }
        catch (error) {
